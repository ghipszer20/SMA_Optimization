...
print(f"Data range: {data.index[0].date()} to {data.index[-1].date()} (full available history)")
...
if SAVE_RESULTS_OPTION:
    ...
    if save_csv == "y":
        filename = f"{TICKER.replace('-', '_')}_sma_{SMA_MIN}_{SMA_MAX}_full_history_results.csv"
